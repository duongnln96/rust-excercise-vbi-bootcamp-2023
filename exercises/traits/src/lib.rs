...
fn static_dispatch<T: Foo>(x: T) {
    // Static dispatch is used when the type is known at compile-time, and the method is called directly on the type
    x.method();
}

// Implement below with trait objects and parameters
fn dynamic_dispatch(x: &dyn Foo) {
    // Dynamic dispatch is used when the type is unknown at compile-time and the method is called on a trait object.
    x.method();
}
